...
# settings
mu_means = [2.0, 0.3, math.pi, 1.0]
mu_stds = [0.3, 0.15, 0.25*math.pi, 0.25]
PCE_ORDER = 3
...
# error vs h
def error_vs_h(N_list=[4,8,16,32,64], mc_samples=5000):
    ...
    index_from_multi = {mi: i for i,mi in enumerate(multi_indices)}
    mc_N = max(N_list)*6
    x_mc, m_mc, _ = monte_carlo_solution(mc_N, N_mc=mc_samples)
    errors = []
    hs = []
    for N in N_list:
        nodes = create_mesh(N)
        ...
        from scipy.interpolate import interp1d
        mean_interp = interp1d(nodes, mean_sg, kind='linear', fill_value="extrapolate")
        mean_sg_on_mc = mean_interp(x_mc)

        # H1 error calculation
        # L2 part: function value difference
        dx_mc = x_mc[1] - x_mc[0]  # assuming uniform MC grid
        L2_error_sq = np.trapz((mean_sg_on_mc - m_mc)**2, x_mc)
        
        # H1 seminorm part: derivative difference
        grad_mc = np.gradient(m_mc, dx_mc)
        grad_sg = np.gradient(mean_sg_on_mc, dx_mc)
        H1_seminorm_sq = np.trapz((grad_sg - grad_mc)**2, x_mc)
        
        # H1 error = sqrt(L2^2 + |·|_H1^2)
        err = np.sqrt(L2_error_sq + H1_seminorm_sq)
        errors.append(err)
        h = 2.0 / N
        hs.append(h)
        print(f"N={N}, h={h:.4f}, H1 error={err:.3e}")
    plt.figure()
    plt.plot(np.log(hs), np.log(errors), 'o-', label='log(error) vs log(h)')
    plt.xlabel('h')
    plt.ylabel('log(H1 error)')
    plt.title('H1 Spatial convergence (fixed PCE order)')
    plt.grid(True)
    plt.savefig("figure.pdf")
    plt.show()
    ...
    print(f"Slope of log(error) vs log(h): {slope:.4f}")

if __name__ == "__main__":
    main([4,8,16], mc_elements=100, mc_samples=5000)
    test_normalization()
    error_vs_h(N_list=[4,8,16,32,64], mc_samples=5000)
