...
def solve_sgfem(N_elem: int, order: int):
    ...
    for elem in range(N_elem):
        ...
        for gp, gw in zip(gl_pts, gl_wts):
            ...
            a_vals = ω1_vals + ω2_vals * np.sin(ω3_vals * x_gp)
            integrand = a_vals * Xi_weights
            M_elem += w_x * (Psi_vals.T @ (integrand[:, None] * Psi_vals))
        ...
        # Insert element contributions into global matrix
...
def abs_H1(u_ref, x_ref, u_hat, x_hat):
    """Absolute H1 error = sqrt(L2_error^2 + H1_seminorm^2)"""
    u_interp = np.interp(x_ref, x_hat, u_hat)
    
    L2_term = np.trapz((u_interp - u_ref)**2, x_ref)
    
    dx_ref = x_ref[1] - x_ref[0]
    du_ref_dx = np.gradient(u_ref, dx_ref)
    du_interp_dx = np.gradient(u_interp, dx_ref)
    
    H1_seminorm = np.trapz((du_interp_dx - du_ref_dx)**2, x_ref)
    
    # H1 norm = sqrt(L2^2 + |·|_H1^2)
    return np.sqrt(L2_term + H1_seminorm)

# ======= Case 2: fixed K=3, refine h =======

order_fixed = 3
N_elem_list = [4, 8, 12, 16, 20]
errors_h = []
hs = []
...
for N_elem in N_elem_list:
    ...
    # SGFEM solve
    u_mean_sg, nodes_sg = solve_sgfem(N_elem, order_fixed)

    err = abs_H1(u_mean_mc, nodes_mc, u_mean_sg, nodes_sg)
    errors_h.append(err)

# Convergence plot
plt.figure(figsize=(5,4))
plt.loglog(hs, errors_h, 'o-', label='$H^1$ error')

h_start, h_end = min(hs), max(hs)
y_start, y_end = errors_h[-1], errors_h[0]
plt.loglog([h_end, h_start], [h_end, h_start], 'k--', label='slope 1')


plt.xlabel('mesh size $h$')
plt.ylabel(r'$H^1$ error')
plt.title(f'$H^1$ Convergence w.r.t $h$ (fixed $k={order_fixed}$)')
